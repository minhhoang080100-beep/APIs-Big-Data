"""
Test script for Bulk Quay Volumes API
"""
import requests
import json
...
BULK_QUAY_ENDPOINT = f"{BASE_URL}/api/bulkQuayVolumesCB"
...
def test_get_bulk_quay_volumes(token: str, **filters):
    """Test GET /api/bulkQuayVolumesCB with filters"""
    print("\n" + "=" * 60)
    filter_desc = ", ".join([f"{k}={v}" for k, v in filters.items()]) if filters else "No filters"
    print(f"Testing GET /api/bulkQuayVolumesCB ({filter_desc})")
    print("=" * 60)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['data']:
                print(f"\n📋 Bulk Quay Volume Records (first 3):")
                for i, volume in enumerate(data['data'][:3], 1):
                    ...
                    print(f"      Ship Agent: {volume['shipAgentId']}")
                    print(f"      Cargo Type: {volume['cargoTypeId']}")
                    print(f"      Cargo Category: {volume['cargoCategoryId']}")
                    print(f"      Handling Method: {volume['handlingMethodId']}")
                    print(f"      Ship Class (Direction): {volume['shipClassId']}")
                ...
                print(f"      - cargoGroupCode <> 'Container' (no containers)")
...
def test_date_filters(token: str):
    ...
    data = test_get_bulk_quay_volumes(
        token,
...
def test_ship_filter(token: str):
    ...
    if response.status_code == 200:
        ...
        if data['data'] and data['data'][0]['shipId']:
            ...
            test_get_bulk_quay_volumes(token, shipId=ship_id, limit=5)
        else:
...
def test_without_auth():
    ...
    try:
        response = requests.get(BULK_QUAY_ENDPOINT)
        print(f"\n📥 Response Status: {response.status_code}")
...
if __name__ == "__main__":
    print("🚀 TOS Bulk Quay Volumes API - Test Suite")
    ...
    # Test 2: Get all bulk quay volumes (default pagination)
    test_get_bulk_quay_volumes(token)
    
    # Test 3: Get volumes with custom pagination
    test_get_bulk_quay_volumes(token, page=1, limit=10)
    ...
    if response.status_code == 200:
        ...
        if data['data'] and data['data'][0]['handlingMethodId']:
            ...
            test_get_bulk_quay_volumes(token, handlingMethodId=handling_method, limit=5)
        else:
